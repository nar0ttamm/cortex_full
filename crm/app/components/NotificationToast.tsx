        ...
        <div
          ...
          style={{
            background: t.type === 'appointment'
              ? 'linear-gradient(135deg, var(--accent) 0%, #9a3210 100%)'
              : 'linear-gradient(135deg, var(--bg-ink) 0%, #241f1a 100%)',
            borderColor: t.type === 'appointment' ? 'rgba(255,255,255,0.12)' : 'var(--border)',
          }}
        >
          <div           className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 text-sm font-bold ${
            t.type === 'appointment' ? 'bg-white/15' : 'bg-[var(--accent)]'
          }`}>
          ...
          <div className="flex-1 min-w-0">
            {t.type === 'appointment' ? (
              <>
                <p className="text-xs font-bold text-white/80 mb-0.5">Appointment Booked</p>
              ...
              <>
                <p className="text-xs font-bold text-[var(--accent)] mb-0.5">New Lead</p>
                <p className="text-sm font-semibold text-white truncate">{t.name}</p>
